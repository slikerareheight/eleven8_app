...
if($bookingId<1 || $reference==='') json_response(['success'=>false,'message'=>'Invalid booking or payment reference.'],422);
...
$stmt=$pdo->prepare('SELECT id,user_id,email,amount,status,payment_reference FROM bookings WHERE id=? LIMIT 1');
...
if(!empty($_SESSION['user_id']) && $booking['user_id'] !== null && (int)$booking['user_id'] !== (int)$_SESSION['user_id']) {
    json_response(['success'=>false,'message'=>'You are not authorized to verify this booking.'],403);
}
...
json_response(['success'=>true,'message'=>'Payment verified successfully.','booking_id'=>$bookingId,'reference'=>$reference]);
