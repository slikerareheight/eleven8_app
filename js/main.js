...
(() => {
  ...
  function initLogin(){const form=$("#loginForm");if(!form)return;form.addEventListener("submit",async e=>{e.preventDefault();const status=$("#loginStatus");try{const csrf=await getCsrf();const r=await api("login.php",{csrf,email:form.elements.Email.value.trim(),password:form.elements.Password.value});localStorage.setItem(SESSION_KEY,JSON.stringify(r.user));logActivity("Login success",{Email:r.user.email});status.textContent="Welcome back, "+r.user.first_name+". Login successful.";status.className="form-status success";setTimeout(()=>location.href="booking.html",700)}catch(err){logActivity("Login attempt",{Email:form.elements.Email.value.trim(),result:"failed"});status.textContent=err.message;status.className="form-status error"}})}
